...
# function check date not null to convet datetime
def check_date(date):
    if (date is not None):
...
def generate_random_string(length):
    #create string include letters and digits
    alphabet = string.ascii_letters + string.digits
    return ''.join(secrets.choice(alphabet) for _ in range(length))

def generate_hmac_sha512(key, message):
    """
    Generates HMAC-SHA512 authentication token.
    
    Args:
    - key (str): The secret key for HMAC.
    - message (str): The message to be authenticated.
    
    Returns:
    - str: The HMAC-SHA512 authentication token.
    """
    hmac_obj = hmac.new(key.encode('utf-8'), message.encode('utf-8'), hashlib.sha512)
